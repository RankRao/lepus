...
func ChannelList(c *gin.Context) {
	...
	if method == "POST" {
		...
		c.BindJSON(&params)
		fmt.Print(params)
		data := model.AlarmChannel{Name: params["name"].(string), Description: params["description"].(string), MailEnable: int(params["mail_enable"].(float64)), Enable: int(params["enable"].(float64)), MailList: params["mail_list"].(string)}
	...
	if method == "PUT" {
		params := make(map[string]interface{})
		c.BindJSON(&params)
		result := db.Model(&model.AlarmChannel{}).Where("id=?", params["id"].(float64)).Updates(map[string]interface{}{"Name": params["name"].(string), "Description": params["description"].(string), "Enable": params["enable"].(float64), "MailEnable": params["mail_enable"].(float64), "MailList": params["mail_list"].(string)})
		if result.Error != nil {
